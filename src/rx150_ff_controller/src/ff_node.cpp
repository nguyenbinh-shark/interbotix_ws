...
#include "rx150_ff_controller/ff_node.hpp"
#include "fuzzy_type1.h"   // extern "C": float fuzzy_type1_eval(float e, float ed)

// ff_node: cấu trúc giống fuzzy_node (fuzzy PD + profile Ruckig TOTG), nhưng feedforward
// là vận tốc + gia tốc từ profile thay cho bù trọng lực:
//   PWM_ff = Kv · q̇_profile + Ka · q̈_profile   (đơn vị PWM trực tiếp; =0 khi đứng yên).
// KHÔNG dùng Pinocchio/URDF.

FFNode::FFNode() : rclcpp::Node("ff_node") {
  // 1. Parameters
  ...
  // --- Feedforward vận tốc + gia tốc từ profile (thay cho bù trọng lực) ---
  Kv_ = this->declare_parameter<std::vector<double>>("Kv", std::vector<double>());
  Ka_ = this->declare_parameter<std::vector<double>>("Ka", std::vector<double>());
  ...
  cli_info_->async_send_request(
      req, std::bind(&FFNode::onRobotInfo, this, std::placeholders::_1));
  ...
  // Topic tương đối -> /rx150/ff/setpoint. QoS reliable để không mất lệnh.
  sub_setpoint_ = this->create_subscription<std_msgs::msg::Float64MultiArray>(
      "ff/setpoint", 10,
      std::bind(&FFNode::onSetpoint, this, std::placeholders::_1));
  pub_err_ = this->create_publisher<sensor_msgs::msg::JointState>("ff/error", rclcpp::SensorDataQoS());
  pub_edot_ = this->create_publisher<sensor_msgs::msg::JointState>("ff/edot", rclcpp::SensorDataQoS());
  pub_eff_ = this->create_publisher<sensor_msgs::msg::JointState>("ff/effort", rclcpp::SensorDataQoS());
  pub_ref_ = this->create_publisher<sensor_msgs::msg::JointState>(
      "ff/reference", rclcpp::SensorDataQoS());  // profile q_ref/qdot_ref để plot
  pub_ff_ = this->create_publisher<sensor_msgs::msg::JointState>("ff/feedforward", rclcpp::SensorDataQoS());
  ...
  timer_ = this->create_wall_timer(
      ...
      std::bind(&FFNode::onTimer, this));

  // 6b. Live-tuning: ros2 param set Ke/Ked/Ku/u_max/Kv/Ka cập nhật thẳng vào vòng điều khiển.
  // Đăng ký SAU khi các declare_parameter ở mục 1 hoàn tất (chỉ set runtime mới trigger).
  param_cb_handle_ = this->add_on_set_parameters_callback(
      std::bind(&FFNode::onParamChange, this, std::placeholders::_1));
...
void FFNode::onRobotInfo(
  ...
  // (Không cần GravityCompensation — ff_node bỏ bù trọng lực, feedforward là Kv·q̇+Ka·q̈.)
  ...
  if (Ke_.size() != n || Ked_.size() != n || Ku_.size() != n || u_max_.size() != n ||
      reference_.size() != n || Kv_.size() != n || Ka_.size() != n) {
    RCLCPP_ERROR(
        this->get_logger(),
        "param size mismatch: joints=%zu, Ke=%zu, Ked=%zu, Ku=%zu, u_max=%zu, reference=%zu, Kv=%zu, Ka=%zu",
        n, Ke_.size(), Ked_.size(), Ku_.size(), u_max_.size(), reference_.size(), Kv_.size(), Ka_.size());
  ...
  ready_ = true;
  RCLCPP_INFO(this->get_logger(), "ff ready: %zu joints, pwm mode%s, vel/acc feedforward",
      n, enable_profile_ ? ", profile TOTG on" : "");
}

void FFNode::configureProfile() {
...
void FFNode::onSetpoint(const std_msgs::msg::Float64MultiArray::SharedPtr msg) {
  ...
  // Nếu profile đã cấu hình, cập nhật target để Ruckig tiến về setpoint mới.
...
rcl_interfaces::msg::SetParametersResult FFNode::onParamChange(
    const std::vector<rclcpp::Parameter> & params) {
  // Live-tuning: chỉ Ke/Ked/Ku/u_max/Kv/Ka (mảng double, đúng số khớp) có hiệu lực online.
  rcl_interfaces::msg::SetParametersResult result;
  result.successful = true;
  for (const auto & p : params) {
    const std::string & name = p.get_name();
    const bool is_gain = (name == "Ke" || name == "Ked" || name == "Ku" ||
                          name == "u_max" || name == "Kv" || name == "Ka");
    if (!is_gain) {
    ...
    else if (name == "Kv") Kv_ = v;
    else if (name == "Ka") Ka_ = v;
...
void FFNode::onTimer() {
  ...
  sensor_msgs::msg::JointState ff_msg;
  ff_msg.header.stamp = stamp;
  ff_msg.name = joint_names_;
  ff_msg.effort.assign(n, 0.0);
  ...
  if (enable_profile_ && profile_configured_ && otg_) {
    ...
    if (res == ruckig::Result::Working || res == ruckig::Result::Finished) {
      ...
      for (size_t i = 0; i < kProfileDoF; ++i) {
        ...
        qddot_ref_[i] = otg_out_.new_acceleration[i];   // MỚI: capture gia tốc profile cho FF
      }
    } else {
      for (size_t i = 0; i < kProfileDoF; ++i) {
  ...
  for (size_t i = 0; i < n; ++i) {
    ...
    // --- Feedforward vận tốc + gia tốc từ profile (thay cho bù trọng lực) ---
    // Đơn vị PWM trực tiếp; =0 khi đứng yên (q̇=q̈=0) -> arm chỉ được giữ bởi fuzzy PD.
    double ff_pwm = 0.0;
    if (i < kProfileDoF) {
      ff_pwm = Kv_[i] * qdot_ref_[i] + Ka_[i] * qddot_ref_[i];
    }
    u += ff_pwm;
...
int main(int argc, char ** argv) {
  ...
  rclcpp::spin(std::make_shared<FFNode>());
  rclcpp::shutdown();
  return 0;
}
